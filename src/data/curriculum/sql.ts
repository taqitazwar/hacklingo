import { Language } from '../../types';
import { Colors } from '../../constants';

const sqlSection1 = {
  id: 'sql-select',
  title: 'SELECT Basics',
  description: 'Query data with SELECT, WHERE, and ORDER BY.',
  accentColor: Colors.blue,
  order: 1,
  lessons: [
    {
      id: 'sql-select-intro',
      sectionId: 'sql-select',
      title: 'SELECT & FROM',
      description: 'Retrieve data from a table.',
      lessonType: 'standard' as const,
      order: 1,
      completionXpBonus: 12,
      challenges: [
        {
          id: 'sql-s-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'Which clause retrieves all columns?',
          difficulty: 'easy' as const,
          question: 'Which SQL query selects all columns from a table named users?',
          options: [
            'GET * FROM users',
            'SELECT * FROM users',
            'FETCH ALL FROM users',
            'READ * users',
          ],
          correctAnswer: 'SELECT * FROM users',
          explanation: 'SELECT * FROM table_name retrieves all columns. The * wildcard means "all columns".',
        },
        {
          id: 'sql-s-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Select only the name column',
          difficulty: 'easy' as const,
          codeWithBlank: 'SELECT ___ FROM employees;',
          options: ['name', '*', 'ALL', 'column'],
          correctAnswer: 'name',
          explanation: 'List specific column names after SELECT to retrieve only those columns.',
        },
        {
          id: 'sql-s-3',
          type: 'FILL_BLANK' as const,
          instruction: 'Filter rows with a condition',
          difficulty: 'easy' as const,
          codeWithBlank: 'SELECT * FROM products ___ price > 100;',
          options: ['WHERE', 'WHEN', 'IF', 'HAVING'],
          correctAnswer: 'WHERE',
          explanation: 'WHERE filters rows based on a condition. Only rows where the condition is true are returned.',
        },
        {
          id: 'sql-s-4',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'Which clause sorts results?',
          difficulty: 'easy' as const,
          question: 'Which clause sorts query results in SQL?',
          options: ['SORT BY', 'ORDER BY', 'ARRANGE BY', 'RANK BY'],
          correctAnswer: 'ORDER BY',
          explanation: 'ORDER BY sorts results. Add ASC for ascending (default) or DESC for descending.',
        },
        {
          id: 'sql-s-5',
          type: 'FIX_BUG' as const,
          instruction: 'Fix the SQL query',
          difficulty: 'medium' as const,
          buggyCode: 'SELECT name, age\nFROM users\nWHERE age > 18\nORDER age DESC;',
          bugLineIndex: 3,
          options: [
            'ORDER BY age DESC;',
            'ORDER BY age;',
            'SORT BY age DESC;',
            'ORDER age ASC;',
          ],
          correctAnswer: 'ORDER BY age DESC;',
          explanation: 'The correct keyword is ORDER BY, not just ORDER. The BY is required.',
        },,
        {
          id: 'sql-select-extra-1',
          type: 'FILL_BLANK',
          question: "Select distinct values",
          codeSnippet: "SELECT ___ department FROM employees;",
          correctAnswer: 'DISTINCT',
          explanation: 'DISTINCT filters out duplicate values from results.',
          xpReward: 10,
          difficulty: 'easy',
        },
        {
          id: 'sql-sel-extra-2',
          type: 'PREDICT_OUTPUT',
          question: "What does this return?",
          codeSnippet: "SELECT 2 + 2 AS result;",
          correctAnswer: '4',
          explanation: 'SQL can evaluate expressions directly in SELECT.',
          xpReward: 10,
          difficulty: 'easy',
        },
        {
          id: 'sql-sel-extra-3',
          type: 'MULTIPLE_CHOICE',
          question: "Which clause sorts query results?",
          options: ['SORT BY', 'GROUP BY', 'ORDER BY', 'ARRANGE BY'],
          correctAnswer: 'ORDER BY',
          explanation: 'ORDER BY sorts results. Add DESC for descending order.',
          xpReward: 15, difficulty: 'easy',
        },
        {
          id: 'sql-sel-extra-4',
          type: 'PREDICT_OUTPUT',
          question: "What does this return?",
          codeSnippet: "SELECT LOWER('HELLO WORLD');",
          correctAnswer: 'hello world',
          explanation: 'LOWER() converts a string to lowercase.',
          xpReward: 10, difficulty: 'easy',
        },
        {
          id: 'sql-sel-extra-5',
          type: 'FILL_BLANK',
          question: "Limit results to 5 rows",
          codeSnippet: "SELECT * FROM products ORDER BY price DESC ___ 5;",
          correctAnswer: 'LIMIT',
          explanation: 'LIMIT n restricts the result set to n rows.',
          xpReward: 10, difficulty: 'easy',
        },
        {
          id: 'sql-sel-extra-6',
          type: 'PREDICT_OUTPUT',
          question: "What does this return?",
          codeSnippet: "SELECT ROUND(3.7654, 2);",
          correctAnswer: '3.77',
          explanation: 'ROUND(n, d) rounds to d decimal places. 3.7654 → 3.77.',
          xpReward: 10, difficulty: 'easy',
        }
      ],
    },
    {
      id: 'sql-limit',
      sectionId: 'sql-select',
      title: 'LIMIT & DISTINCT',
      description: 'Control the number of results and remove duplicates.',
      lessonType: 'standard' as const,
      order: 2,
      completionXpBonus: 12,
      challenges: [
        {
          id: 'sql-ld-1',
          type: 'FILL_BLANK' as const,
          instruction: 'Return only the first 10 rows',
          difficulty: 'easy' as const,
          codeWithBlank: 'SELECT * FROM products\nORDER BY price DESC\n___ 10;',
          options: ['LIMIT', 'TOP', 'MAX', 'TAKE'],
          correctAnswer: 'LIMIT',
          explanation: 'LIMIT restricts the number of rows returned. LIMIT 10 returns the first 10 rows after ordering.',
        },
        {
          id: 'sql-ld-2',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does DISTINCT do?',
          difficulty: 'easy' as const,
          question: 'What does SELECT DISTINCT do?',
          options: [
            'Sorts the results',
            'Returns only unique values, removing duplicates',
            'Filters null values',
            'Returns the first row only',
          ],
          correctAnswer: 'Returns only unique values, removing duplicates',
          explanation: 'SELECT DISTINCT removes duplicate rows from the result set.',
        },
        {
          id: 'sql-ld-3',
          type: 'FIX_BUG' as const,
          instruction: 'Fix the LIMIT clause placement',
          difficulty: 'medium' as const,
          buggyCode: 'SELECT * LIMIT 5\nFROM users\nWHERE active = 1;',
          bugLineIndex: 0,
          options: [
            'SELECT * FROM users WHERE active = 1 LIMIT 5;',
            'LIMIT 5 SELECT * FROM users WHERE active = 1;',
            'SELECT LIMIT 5 * FROM users WHERE active = 1;',
            'SELECT * FROM users LIMIT 5 WHERE active = 1;',
          ],
          correctAnswer: 'SELECT * FROM users WHERE active = 1 LIMIT 5;',
          explanation: 'SQL clause order: SELECT, FROM, WHERE, ORDER BY, LIMIT. LIMIT goes at the end.',
        },
        {
          id: 'sql-ld-4',
          type: 'FILL_BLANK' as const,
          instruction: 'Use LIKE to search for a pattern',
          difficulty: 'medium' as const,
          codeWithBlank: 'SELECT * FROM products\nWHERE name ___ "Phone%";',
          options: ['LIKE', 'MATCHES', 'CONTAINS', '='],
          correctAnswer: 'LIKE',
          explanation: 'LIKE is used for pattern matching. % matches zero or more characters.',
        },
        {
          id: 'sql-ld-5',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does IN() operator do?',
          difficulty: 'medium' as const,
          question: 'What does WHERE status IN ("active", "pending") do?',
          options: [
            'Checks if status equals both values',
            'Matches rows where status equals any value in the list',
            'Combines two WHERE clauses with AND',
            'Creates a subquery',
          ],
          correctAnswer: 'Matches rows where status equals any value in the list',
          explanation: 'IN() is shorthand for multiple OR conditions. It matches if status = "active" OR status = "pending".',
        },
      ],
    },
    {
      id: 'sql-functions',
      sectionId: 'sql-select',
      title: 'String & Date Functions',
      description: 'UPPER, LOWER, TRIM, DATE functions.',
      lessonType: 'standard' as const,
      order: 3,
      completionXpBonus: 14,
      challenges: [
        {
          id: 'sql-fn-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does UPPER() do?',
          difficulty: 'easy' as const,
          question: 'What does SELECT UPPER("hello") return?',
          options: ['hello', 'HELLO', 'Hello', 'Error'],
          correctAnswer: 'HELLO',
          explanation: 'UPPER() converts all characters to uppercase. LOWER() does the reverse.',
        },
        {
          id: 'sql-fn-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Remove leading/trailing spaces',
          difficulty: 'easy' as const,
          codeWithBlank: 'SELECT ___("  hello  ") AS clean_name;',
          options: ['TRIM', 'STRIP', 'CLEAN', 'REMOVE'],
          correctAnswer: 'TRIM',
          explanation: 'TRIM() removes leading and trailing whitespace. LTRIM/RTRIM remove only left or right spaces.',
        },
        {
          id: 'sql-fn-3',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does COALESCE do?',
          difficulty: 'medium' as const,
          question: 'What does COALESCE(a, b, c) return?',
          options: [
            'The sum of all arguments',
            'The first non-NULL value from the list',
            'The last argument always',
            'NULL if any argument is NULL',
          ],
          correctAnswer: 'The first non-NULL value from the list',
          explanation: 'COALESCE returns the first non-NULL value. Useful for providing default values: COALESCE(email, phone, "unknown").',
        },
        {
          id: 'sql-fn-4',
          type: 'FILL_BLANK' as const,
          instruction: 'Concatenate two strings',
          difficulty: 'medium' as const,
          codeWithBlank: 'SELECT ___(first_name, " ", last_name) AS full_name FROM users;',
          options: ['CONCAT', 'JOIN', 'MERGE', 'APPEND'],
          correctAnswer: 'CONCAT',
          explanation: 'CONCAT() joins strings together. CONCAT("Hello", " ", "World") returns "Hello World".',
        },
        {
          id: 'sql-fn-5',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'How do you get the current date?',
          difficulty: 'medium' as const,
          question: 'Which SQL function returns the current date?',
          options: ['NOW()', 'TODAY()', 'CURRENT_DATE', 'DATE()'],
          correctAnswer: 'CURRENT_DATE',
          explanation: 'CURRENT_DATE returns todays date. NOW() or CURRENT_TIMESTAMP returns date and time.',
        },,
        {
          id: 'sql-func-extra-1',
          type: 'PREDICT_OUTPUT',
          question: "What does this return?",
          codeSnippet: "SELECT UPPER('hello world');",
          correctAnswer: 'HELLO WORLD',
          explanation: 'UPPER() converts a string to all uppercase.',
          xpReward: 10, difficulty: 'easy',
        }
      ],
    },
    {
      id: 'sql-transactions',
      sectionId: 'sql-select',
      title: 'Transactions',
      description: 'ACID properties and transaction control with COMMIT and ROLLBACK.',
      lessonType: 'standard' as const,
      order: 4,
      completionXpBonus: 16,
      challenges: [
        {
          id: 'sql-tx-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does ACID stand for?',
          difficulty: 'medium' as const,
          question: 'What does the A in ACID stand for?',
          options: ['Automated', 'Atomic', 'Accessible', 'Aligned'],
          correctAnswer: 'Atomic',
          explanation: 'ACID: Atomicity (all or nothing), Consistency, Isolation, Durability.',
        },
        {
          id: 'sql-tx-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Start a transaction',
          difficulty: 'medium' as const,
          codeWithBlank: '___ TRANSACTION;\nINSERT INTO orders VALUES (1, 100);\nCOMMIT;',
          options: ['BEGIN', 'START', 'OPEN', 'INIT'],
          correctAnswer: 'BEGIN',
          explanation: 'BEGIN TRANSACTION (or just BEGIN) starts a transaction block. Changes are not saved until COMMIT.',
        },
        {
          id: 'sql-tx-3',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does ROLLBACK do?',
          difficulty: 'medium' as const,
          question: 'What does ROLLBACK do in SQL?',
          options: [
            'Commits the transaction permanently',
            'Undoes all changes made since the transaction began',
            'Creates a backup of the database',
            'Moves to the previous version of the schema',
          ],
          correctAnswer: 'Undoes all changes made since the transaction began',
          explanation: 'ROLLBACK cancels all changes in the current transaction. Used when an error occurs.',
        },
        {
          id: 'sql-tx-4',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is Isolation?',
          difficulty: 'hard' as const,
          question: 'What does Isolation mean in ACID?',
          options: [
            'Transactions are stored separately from data',
            'Concurrent transactions execute as if they were sequential',
            'Each table is isolated from others',
            'Queries run in separate threads',
          ],
          correctAnswer: 'Concurrent transactions execute as if they were sequential',
          explanation: 'Isolation prevents dirty reads, non-repeatable reads, and phantom reads between concurrent transactions.',
        },
        {
          id: 'sql-tx-5',
          type: 'FILL_BLANK' as const,
          instruction: 'Save a savepoint within a transaction',
          difficulty: 'hard' as const,
          codeWithBlank: 'BEGIN;\nINSERT INTO accounts VALUES (1, 500);\nSAVEPOINT ___;
INSERT INTO accounts VALUES (2, 300);\nROLLBACK TO step1;',
          options: ['step1', 'checkpoint', 'mark1', 'save1'],
          correctAnswer: 'step1',
          explanation: 'SAVEPOINT creates a point within a transaction to roll back to without canceling the entire transaction.',
        },,
        {
          id: 'sql-txn-extra-1',
          type: 'MULTIPLE_CHOICE',
          question: "What does ROLLBACK do?",
          options: [
            'Saves changes permanently',
            'Undoes all changes in the current transaction',
            'Deletes all rows',
            'Creates a savepoint'
          ],
          correctAnswer: 'Undoes all changes in the current transaction',
          explanation: 'ROLLBACK reverts all changes made since the last COMMIT or BEGIN TRANSACTION.',
          xpReward: 15, difficulty: 'medium',
        }
      ],
    },
  ],
};

const sqlSection2 = {
  id: 'sql-joins',
  title: 'Joins',
  description: 'Combine data from multiple tables with JOINs.',
  accentColor: Colors.teal,
  order: 2,
  lessons: [
    {
      id: 'sql-joins-intro',
      sectionId: 'sql-joins',
      title: 'INNER JOIN',
      description: 'Combine rows from two tables on a matching key.',
      lessonType: 'standard' as const,
      order: 1,
      completionXpBonus: 14,
      challenges: [
        {
          id: 'sql-j-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does INNER JOIN return?',
          difficulty: 'easy' as const,
          question: 'What rows does an INNER JOIN return?',
          options: [
            'All rows from the left table',
            'All rows from both tables',
            'Only rows with matching values in both tables',
            'Rows with no match in either table',
          ],
          correctAnswer: 'Only rows with matching values in both tables',
          explanation: 'INNER JOIN returns rows where the join condition is true in both tables.',
        },
        {
          id: 'sql-j-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Complete the JOIN query',
          difficulty: 'medium' as const,
          codeWithBlank: 'SELECT o.id, u.name\nFROM orders o\n___ JOIN users u ON o.user_id = u.id;',
          options: ['INNER', 'OUTER', 'LEFT', 'CROSS'],
          correctAnswer: 'INNER',
          explanation: 'INNER JOIN is the most common join type. It returns only matching rows from both tables.',
        },
        {
          id: 'sql-j-3',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does LEFT JOIN include?',
          difficulty: 'medium' as const,
          question: 'What does a LEFT JOIN return that an INNER JOIN does not?',
          options: [
            'Nothing different — they are identical',
            'All rows from the right table, even without a match',
            'All rows from the left table, even without a match',
            'Only rows with no match in either table',
          ],
          correctAnswer: 'All rows from the left table, even without a match',
          explanation: 'LEFT JOIN keeps all rows from the left table. Unmatched right-table columns are NULL.',
        },
        {
          id: 'sql-j-4',
          type: 'FIX_BUG' as const,
          instruction: 'Fix the JOIN condition',
          difficulty: 'medium' as const,
          buggyCode: 'SELECT p.title, c.name\nFROM posts p\nINNER JOIN categories c ON p.id = c.id;',
          bugLineIndex: 2,
          options: [
            'INNER JOIN categories c ON p.category_id = c.id;',
            'INNER JOIN categories c ON p.id = c.post_id;',
            'INNER JOIN categories c = p.category ON c.id;',
            'JOIN categories c WHERE p.category = c.name;',
          ],
          correctAnswer: 'INNER JOIN categories c ON p.category_id = c.id;',
          explanation: 'The join should use p.category_id (the foreign key on posts) to match c.id (the primary key on categories).',
        },
        {
          id: 'sql-j-5',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'When would you use a LEFT JOIN?',
          difficulty: 'hard' as const,
          question: 'You want all customers, even those with no orders. Which JOIN do you use?',
          options: [
            'INNER JOIN orders ON customers.id = orders.customer_id',
            'RIGHT JOIN orders ON customers.id = orders.customer_id',
            'LEFT JOIN orders ON customers.id = orders.customer_id',
            'FULL JOIN orders ON customers.id = orders.customer_id',
          ],
          correctAnswer: 'LEFT JOIN orders ON customers.id = orders.customer_id',
          explanation: 'LEFT JOIN keeps all customers (left table). Customers with no orders will have NULL in orders columns.',
        },,
        {
          id: 'sql-join-extra-1',
          type: 'PREDICT_OUTPUT',
          question: "What does INNER JOIN return?",
          codeSnippet: "-- Table A: 1, 2, 3\n-- Table B: 2, 3, 4\nSELECT * FROM A INNER JOIN B ON A.id = B.id",
          correctAnswer: 'Rows where id is 2 and 3',
          explanation: 'INNER JOIN returns only rows with matching values in both tables.',
          xpReward: 10,
          difficulty: 'medium',
        },
        {
          id: 'sql-join-extra-2',
          type: 'FILL_BLANK',
          question: "Write a LEFT JOIN",
          codeSnippet: "SELECT * FROM customers\n___ JOIN orders ON customers.id = orders.customer_id;",
          correctAnswer: 'LEFT',
          explanation: 'LEFT JOIN returns all rows from the left table plus matched rows from the right.',
          xpReward: 10,
          difficulty: 'medium',
        },
        {
          id: 'sql-join-extra-3',
          type: 'MULTIPLE_CHOICE',
          question: "What does a FULL OUTER JOIN return?",
          options: [
            'Only matching rows from both tables',
            'All rows from left table',
            'All rows from both tables with NULLs for non-matches',
            'Cartesian product'
          ],
          correctAnswer: 'All rows from both tables with NULLs for non-matches',
          explanation: 'FULL OUTER JOIN returns all rows from both tables. Unmatched rows get NULL in the opposite table's columns.',
          xpReward: 15, difficulty: 'medium',
        },
        {
          id: 'sql-join-extra-4',
          type: 'PREDICT_OUTPUT',
          question: "What does a CROSS JOIN produce?",
          codeSnippet: "-- Table A: 3 rows, Table B: 4 rows
SELECT COUNT(*) FROM A CROSS JOIN B;",
          correctAnswer: '12',
          explanation: 'CROSS JOIN produces a Cartesian product: every row from A with every row from B. 3×4=12.',
          xpReward: 15, difficulty: 'medium',
        },
        {
          id: 'sql-join-extra-5',
          type: 'FILL_BLANK',
          question: "Write a self-join to find employees with same manager",
          codeSnippet: "SELECT a.name FROM employees a ___ employees b ON a.manager_id = b.manager_id WHERE a.id != b.id;",
          correctAnswer: 'JOIN',
          explanation: 'A self-join joins a table to itself. Use aliases (a, b) to distinguish the two instances.',
          xpReward: 20, difficulty: 'hard',
        }
      ],
    },
    {
      id: 'sql-outer-joins',
      sectionId: 'sql-joins',
      title: 'Outer Joins',
      description: 'RIGHT JOIN, FULL OUTER JOIN, and CROSS JOIN.',
      lessonType: 'standard' as const,
      order: 2,
      completionXpBonus: 16,
      challenges: [
        {
          id: 'sql-oj-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does RIGHT JOIN return?',
          difficulty: 'medium' as const,
          question: 'What does a RIGHT JOIN return that an INNER JOIN does not?',
          options: [
            'All rows from both tables',
            'All rows from the right table, even without a match in the left',
            'Only matching rows from both tables',
            'Rows sorted by the right table',
          ],
          correctAnswer: 'All rows from the right table, even without a match in the left',
          explanation: 'RIGHT JOIN keeps all rows from the right table. Unmatched left-table columns are NULL.',
        },
        {
          id: 'sql-oj-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Write a FULL OUTER JOIN',
          difficulty: 'hard' as const,
          codeWithBlank: 'SELECT * FROM a\n___ OUTER JOIN b ON a.id = b.a_id;',
          options: ['FULL', 'BOTH', 'ALL', 'COMPLETE'],
          correctAnswer: 'FULL',
          explanation: 'FULL OUTER JOIN returns all rows from both tables, with NULLs where there is no match.',
        },
        {
          id: 'sql-oj-3',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is a CROSS JOIN?',
          difficulty: 'hard' as const,
          question: 'What does a CROSS JOIN produce?',
          options: [
            'Rows where columns are equal',
            'The Cartesian product of both tables',
            'Rows with no matches',
            'An error if tables have different columns',
          ],
          correctAnswer: 'The Cartesian product of both tables',
          explanation: 'CROSS JOIN produces n*m rows where n and m are the row counts of each table. No ON condition needed.',
        },
        {
          id: 'sql-oj-4',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'How many rows from this LEFT JOIN?',
          difficulty: 'medium' as const,
          question: 'Table A has 3 rows, Table B has 2 rows, only 1 row matches. How many rows does LEFT JOIN return?',
          options: ['1', '2', '3', '5'],
          correctAnswer: '3',
          explanation: 'LEFT JOIN returns all rows from the left table. The 2 unmatched rows get NULLs for right table columns.',
        },
        {
          id: 'sql-oj-5',
          type: 'FIX_BUG' as const,
          instruction: 'Fix the self-join alias',
          difficulty: 'hard' as const,
          buggyCode: 'SELECT e.name, m.name AS manager\nFROM employees e\nJOIN employees ON e.manager_id = employees.id;',
          bugLineIndex: 2,
          options: [
            'JOIN employees m ON e.manager_id = m.id;',
            'JOIN employees AS m ON e.id = m.id;',
            'SELF JOIN employees m ON e.manager_id = m.id;',
            'JOIN employees e2 ON e.id = e2.manager_id;',
          ],
          correctAnswer: 'JOIN employees m ON e.manager_id = m.id;',
          explanation: 'A self-join requires aliases. JOIN employees m gives alias m, then ON e.manager_id = m.id.',
        },
      ],
    },
    {
      id: 'sql-views',
      sectionId: 'sql-joins',
      title: 'Views & CTEs',
      description: 'Create reusable views and common table expressions.',
      lessonType: 'standard' as const,
      order: 3,
      completionXpBonus: 16,
      challenges: [
        {
          id: 'sql-vw-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is a view?',
          difficulty: 'medium' as const,
          question: 'What is a SQL view?',
          options: [
            'A cached copy of query results',
            'A virtual table based on a SELECT query',
            'A database index on multiple columns',
            'A stored procedure',
          ],
          correctAnswer: 'A virtual table based on a SELECT query',
          explanation: 'A view is a named SELECT statement. It looks like a table but computes results on access.',
        },
        {
          id: 'sql-vw-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Create a view',
          difficulty: 'medium' as const,
          codeWithBlank: 'CREATE ___ active_users AS\nSELECT * FROM users WHERE active = 1;',
          options: ['VIEW', 'TABLE', 'QUERY', 'ALIAS'],
          correctAnswer: 'VIEW',
          explanation: 'CREATE VIEW name AS SELECT... defines a virtual table. Query it like a regular table.',
        },
        {
          id: 'sql-vw-3',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is a CTE?',
          difficulty: 'hard' as const,
          question: 'What does CTE stand for in SQL?',
          options: [
            'Column Type Expression',
            'Common Table Expression',
            'Conditional Table Enumeration',
            'Computed Table Entry',
          ],
          correctAnswer: 'Common Table Expression',
          explanation: 'A CTE (WITH clause) is a temporary named result set used within a single query. Improves readability.',
        },
        {
          id: 'sql-vw-4',
          type: 'FILL_BLANK' as const,
          instruction: 'Write a CTE with WITH clause',
          difficulty: 'hard' as const,
          codeWithBlank: '___ top_earners AS (\n  SELECT * FROM employees WHERE salary > 100000\n)\nSELECT * FROM top_earners;',
          options: ['WITH', 'USING', 'DEFINE', 'AS'],
          correctAnswer: 'WITH',
          explanation: 'WITH name AS (query) defines a CTE. The CTE can then be referenced in the main query.',
        },
        {
          id: 'sql-vw-5',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'CTE vs subquery?',
          difficulty: 'hard' as const,
          question: 'What is an advantage of CTEs over subqueries?',
          options: [
            'CTEs are always faster',
            'CTEs can be referenced multiple times in the same query',
            'CTEs do not require SELECT',
            'CTEs replace indexes',
          ],
          correctAnswer: 'CTEs can be referenced multiple times in the same query',
          explanation: 'CTEs improve readability and can be referenced multiple times. Recursive CTEs also enable tree traversal.',
        },,
        {
          id: 'sql-view-extra-1',
          type: 'FILL_BLANK',
          question: "Create a virtual table (view)",
          codeSnippet: "___ VIEW active_users AS
SELECT * FROM users WHERE status = 'active';",
          correctAnswer: 'CREATE',
          explanation: 'CREATE VIEW defines a stored query as a virtual table.',
          xpReward: 15,
          difficulty: 'medium',
        }
      ],
    },
  ],
};

const sqlSection3 = {
  id: 'sql-aggregates',
  title: 'Aggregates',
  description: 'COUNT, SUM, AVG, GROUP BY, and HAVING.',
  accentColor: Colors.orange,
  order: 3,
  lessons: [
    {
      id: 'sql-agg-intro',
      sectionId: 'sql-aggregates',
      title: 'COUNT & SUM',
      description: 'Aggregate functions for counting and summing.',
      lessonType: 'standard' as const,
      order: 1,
      completionXpBonus: 14,
      challenges: [
        {
          id: 'sql-ag-1',
          type: 'FILL_BLANK' as const,
          instruction: 'Count all rows in a table',
          difficulty: 'easy' as const,
          codeWithBlank: 'SELECT ___(*)  FROM orders;',
          options: ['COUNT', 'SUM', 'TOTAL', 'NUM'],
          correctAnswer: 'COUNT',
          explanation: 'COUNT(*) counts all rows in the result set. Use COUNT(column) to count non-NULL values.',
        },
        {
          id: 'sql-ag-2',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'Which function calculates the average?',
          difficulty: 'easy' as const,
          question: 'Which SQL function returns the average value of a numeric column?',
          options: ['MEAN()', 'AVERAGE()', 'AVG()', 'MID()'],
          correctAnswer: 'AVG()',
          explanation: 'AVG(column) returns the arithmetic mean of all non-NULL values in the column.',
        },
        {
          id: 'sql-ag-3',
          type: 'FILL_BLANK' as const,
          instruction: 'Group results by category',
          difficulty: 'medium' as const,
          codeWithBlank: 'SELECT category, COUNT(*)\nFROM products\n___ BY category;',
          options: ['GROUP', 'ORDER', 'SORT', 'CLUSTER'],
          correctAnswer: 'GROUP',
          explanation: 'GROUP BY groups rows sharing the same value into summary rows for aggregate functions.',
        },
        {
          id: 'sql-ag-4',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'Filter grouped results with?',
          difficulty: 'medium' as const,
          question: 'Which clause filters the results of a GROUP BY?',
          options: ['WHERE', 'FILTER', 'HAVING', 'WHEN'],
          correctAnswer: 'HAVING',
          explanation: 'HAVING filters grouped results. WHERE filters before grouping; HAVING filters after.',
        },
        {
          id: 'sql-ag-5',
          type: 'FIX_BUG' as const,
          instruction: 'Fix the aggregate query',
          difficulty: 'hard' as const,
          buggyCode: 'SELECT department, AVG(salary)\nFROM employees\nGROUP BY department\nWHERE AVG(salary) > 50000;',
          bugLineIndex: 3,
          options: [
            'HAVING AVG(salary) > 50000;',
            'FILTER AVG(salary) > 50000;',
            'AND AVG(salary) > 50000;',
            'WHERE salary > 50000;',
          ],
          correctAnswer: 'HAVING AVG(salary) > 50000;',
          explanation: 'WHERE cannot filter on aggregate functions. Use HAVING to filter after GROUP BY.',
        },,
        {
          id: 'sql-agg-extra-1',
          type: 'FILL_BLANK',
          question: "Filter aggregated results",
          codeSnippet: "SELECT department, COUNT(*) FROM employees
GROUP BY department
___ COUNT(*) > 5;",
          correctAnswer: 'HAVING',
          explanation: 'HAVING filters groups after GROUP BY, unlike WHERE which filters rows.',
          xpReward: 10,
          difficulty: 'medium',
        },
        {
          id: 'sql-agg-extra-2',
          type: 'MULTIPLE_CHOICE',
          question: "Which aggregate function returns the number of non-NULL rows?",
          options: ['SUM()', 'AVG()', 'COUNT()', 'MAX()'],
          correctAnswer: 'COUNT()',
          explanation: 'COUNT() counts rows. COUNT(*) counts all rows; COUNT(col) counts non-NULL values.',
          xpReward: 15,
          difficulty: 'easy',
        },
        {
          id: 'sql-agg-extra-3',
          type: 'PREDICT_OUTPUT',
          question: "What does this return?",
          codeSnippet: "-- table: scores(value)
-- rows: 10, 20, 30
SELECT AVG(value) FROM scores;",
          correctAnswer: '20',
          explanation: 'AVG sums all values and divides by count: (10+20+30)/3 = 20.',
          xpReward: 10, difficulty: 'easy',
        },
        {
          id: 'sql-agg-extra-4',
          type: 'PREDICT_OUTPUT',
          question: "What does this return?",
          codeSnippet: "-- table: values(n)
-- rows: 10, NULL, 20, 30
SELECT COUNT(*), COUNT(n) FROM values;",
          correctAnswer: '4, 3',
          explanation: 'COUNT(*) counts all rows (4). COUNT(n) skips NULLs (3).',
          xpReward: 15, difficulty: 'medium',
        },
        {
          id: 'sql-agg-extra-5',
          type: 'FILL_BLANK',
          question: "Find minimum salary",
          codeSnippet: "SELECT ___(salary) FROM employees;",
          correctAnswer: 'MIN',
          explanation: 'MIN() returns the smallest value in a column.',
          xpReward: 10, difficulty: 'easy',
        },
        {
          id: 'sql-agg-extra-6',
          type: 'PREDICT_OUTPUT',
          question: "What does this return?",
          codeSnippet: "-- table: nums(n: 3, 7, 2, 9, 1)
SELECT MAX(n) - MIN(n) FROM nums;",
          correctAnswer: '8',
          explanation: 'MAX=9, MIN=1. 9-1=8 is the range.',
          xpReward: 10, difficulty: 'easy',
        }
      ],
    },
    {
      id: 'sql-subqueries',
      sectionId: 'sql-aggregates',
      title: 'Subqueries',
      description: 'Nest queries inside SELECT, WHERE, and FROM.',
      lessonType: 'standard' as const,
      order: 2,
      completionXpBonus: 16,
      challenges: [
        {
          id: 'sql-sq-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is a subquery?',
          difficulty: 'medium' as const,
          question: 'What is a subquery in SQL?',
          options: [
            'A stored procedure',
            'A query nested inside another query',
            'A view that runs automatically',
            'A query that returns no rows',
          ],
          correctAnswer: 'A query nested inside another query',
          explanation: 'Subqueries are queries within queries. They can appear in SELECT, FROM, or WHERE clauses.',
        },
        {
          id: 'sql-sq-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Filter using a subquery result',
          difficulty: 'medium' as const,
          codeWithBlank: 'SELECT name FROM employees\nWHERE salary > (SELECT ___(salary) FROM employees);',
          options: ['AVG', 'MAX', 'SUM', 'COUNT'],
          correctAnswer: 'AVG',
          explanation: 'The subquery returns the average salary. The outer query returns employees earning above average.',
        },
        {
          id: 'sql-sq-3',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is a correlated subquery?',
          difficulty: 'hard' as const,
          question: 'What makes a subquery correlated?',
          options: [
            'It uses JOINs internally',
            'It references columns from the outer query',
            'It runs only once for the whole query',
            'It is always in the SELECT clause',
          ],
          correctAnswer: 'It references columns from the outer query',
          explanation: 'A correlated subquery references the outer query and runs once per row of the outer query.',
        },
        {
          id: 'sql-sq-4',
          type: 'FILL_BLANK' as const,
          instruction: 'Use EXISTS to check for matching rows',
          difficulty: 'hard' as const,
          codeWithBlank: 'SELECT name FROM customers c\nWHERE ___ (SELECT 1 FROM orders o WHERE o.customer_id = c.id);',
          options: ['EXISTS', 'IN', 'ANY', 'HAS'],
          correctAnswer: 'EXISTS',
          explanation: 'EXISTS returns true if the subquery returns any rows. Often more efficient than IN for large datasets.',
        },
        {
          id: 'sql-sq-5',
          type: 'FIX_BUG' as const,
          instruction: 'Fix the missing subquery alias',
          difficulty: 'hard' as const,
          buggyCode: 'SELECT dept, avg_sal\nFROM (SELECT department AS dept, AVG(salary) AS avg_sal\n      FROM employees GROUP BY department)\nWHERE avg_sal > 60000;',
          bugLineIndex: 2,
          options: [
            '      FROM employees GROUP BY department) AS stats',
            '      FROM employees GROUP BY department) stats;',
            '      FROM employees GROUP BY department)',
            '      FROM employees GROUP BY department) sub;',
          ],
          correctAnswer: '      FROM employees GROUP BY department) AS stats',
          explanation: 'Subqueries in FROM must have an alias. Add AS stats after the closing parenthesis.',
        },,
        {
          id: 'sql-sub-extra-1',
          type: 'PREDICT_OUTPUT',
          question: "What does this return?",
          codeSnippet: "-- employees table has salaries: 50000, 75000, 90000\nSELECT MAX(salary) FROM employees\nWHERE salary < (SELECT MAX(salary) FROM employees);",
          correctAnswer: '75000',
          explanation: 'The subquery gets MAX = 90000. Outer query gets MAX salary below 90000 = 75000.',
          xpReward: 15, difficulty: 'hard',
        }
      ],
    },
    {
      id: 'sql-indexes',
      sectionId: 'sql-aggregates',
      title: 'Indexes & Performance',
      description: 'Speed up queries with indexes. Understand query planning.',
      lessonType: 'standard' as const,
      order: 3,
      completionXpBonus: 16,
      challenges: [
        {
          id: 'sql-idx-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is an index?',
          difficulty: 'medium' as const,
          question: 'What is the purpose of a database index?',
          options: [
            'To store backup copies of data',
            'To speed up data retrieval at the cost of extra storage',
            'To encrypt sensitive columns',
            'To enforce referential integrity',
          ],
          correctAnswer: 'To speed up data retrieval at the cost of extra storage',
          explanation: 'An index is a data structure that enables fast lookups. Like a book index — faster to find than reading every page.',
        },
        {
          id: 'sql-idx-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Create an index on a column',
          difficulty: 'medium' as const,
          codeWithBlank: 'CREATE ___ idx_email ON users(email);',
          options: ['INDEX', 'KEY', 'LOOKUP', 'SEARCH'],
          correctAnswer: 'INDEX',
          explanation: 'CREATE INDEX name ON table(column) creates an index. Speeds up queries filtering on that column.',
        },
        {
          id: 'sql-idx-3',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is a primary key index?',
          difficulty: 'easy' as const,
          question: 'What automatically gets an index in most databases?',
          options: [
            'Every column in the table',
            'The primary key column',
            'Foreign key columns',
            'NULL columns',
          ],
          correctAnswer: 'The primary key column',
          explanation: 'Primary keys automatically get a unique index. This is why lookups by primary key are very fast.',
        },
        {
          id: 'sql-idx-4',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'When do indexes hurt performance?',
          difficulty: 'hard' as const,
          question: 'When do indexes slow down a database operation?',
          options: [
            'During SELECT queries',
            'During INSERT, UPDATE, and DELETE operations',
            'When the table has many columns',
            'During JOIN operations',
          ],
          correctAnswer: 'During INSERT, UPDATE, and DELETE operations',
          explanation: 'Indexes must be updated when data changes. Too many indexes slow down writes. Index only frequently-queried columns.',
        },
        {
          id: 'sql-idx-5',
          type: 'FILL_BLANK' as const,
          instruction: 'Use EXPLAIN to see query plan',
          difficulty: 'hard' as const,
          codeWithBlank: '___ SELECT * FROM users WHERE email = "test@example.com";',
          options: ['EXPLAIN', 'ANALYZE', 'DESCRIBE', 'PLAN'],
          correctAnswer: 'EXPLAIN',
          explanation: 'EXPLAIN shows the query execution plan, revealing if indexes are being used or if a full table scan occurs.',
        },,
        {
          id: 'sql-idx-extra-1',
          type: 'MULTIPLE_CHOICE',
          question: "What is the primary purpose of an index?",
          options: [
            'To enforce uniqueness',
            'To speed up data retrieval queries',
            'To store data more compactly',
            'To create relationships between tables'
          ],
          correctAnswer: 'To speed up data retrieval queries',
          explanation: 'Indexes speed up SELECT queries at the cost of slower INSERT/UPDATE operations.',
          xpReward: 15,
          difficulty: 'medium',
        },
        {
          id: 'sql-idx-extra-2',
          type: 'FILL_BLANK',
          question: "Create an index on email column",
          codeSnippet: "CREATE ___ idx_email ON users(email);",
          correctAnswer: 'INDEX',
          explanation: 'CREATE INDEX name ON table(column) creates an index to speed up queries.',
          xpReward: 10, difficulty: 'medium',
        }
      ],
    },
    {
      id: 'sql-window-functions',
      sectionId: 'sql-aggregates',
      title: 'Window Functions',
      description: 'ROW_NUMBER, RANK, LAG, and LEAD functions.',
      lessonType: 'standard' as const,
      order: 4,
      completionXpBonus: 18,
      challenges: [
        {
          id: 'sql-wf-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is a window function?',
          difficulty: 'hard' as const,
          question: 'What makes a window function different from a regular aggregate?',
          options: [
            'It runs on a separate server window',
            'It computes across a set of rows related to the current row without collapsing them',
            'It only works on sorted data',
            'It requires a subquery',
          ],
          correctAnswer: 'It computes across a set of rows related to the current row without collapsing them',
          explanation: 'Window functions use OVER() to define a window of rows. Unlike GROUP BY, they do not collapse rows.',
        },
        {
          id: 'sql-wf-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Assign a row number to each row',
          difficulty: 'hard' as const,
          codeWithBlank: 'SELECT name, salary,\n    ROW_NUMBER() ___ (ORDER BY salary DESC) AS rank\nFROM employees;',
          options: ['OVER', 'WITHIN', 'PARTITION', 'BY'],
          correctAnswer: 'OVER',
          explanation: 'OVER() defines the window. ROW_NUMBER() OVER(ORDER BY salary DESC) numbers rows by salary rank.',
        },
        {
          id: 'sql-wf-3',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is PARTITION BY?',
          difficulty: 'hard' as const,
          question: 'What does PARTITION BY do in a window function?',
          options: [
            'Splits the table into separate tables',
            'Resets the window calculation for each group of rows with the same value',
            'Limits the number of rows in the window',
            'Sorts rows within the window',
          ],
          correctAnswer: 'Resets the window calculation for each group of rows with the same value',
          explanation: 'PARTITION BY is like GROUP BY for window functions. It restarts numbering/aggregation per partition.',
        },
        {
          id: 'sql-wf-4',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What does LAG() do?',
          difficulty: 'hard' as const,
          question: 'What does LAG(column, 1) return?',
          options: [
            'The next row's value',
            'The previous row's value',
            'The minimum value in the window',
            'The row count lag',
          ],
          correctAnswer: 'The previous row's value',
          explanation: 'LAG(col, n) returns the value n rows before the current row. LEAD(col, n) returns n rows ahead.',
        },
        {
          id: 'sql-wf-5',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'RANK vs DENSE_RANK?',
          difficulty: 'hard' as const,
          question: 'What is the difference between RANK() and DENSE_RANK()?',
          options: [
            'No difference — they are identical',
            'RANK() leaves gaps after ties; DENSE_RANK() does not',
            'DENSE_RANK() leaves gaps; RANK() does not',
            'RANK() only works with integers',
          ],
          correctAnswer: 'RANK() leaves gaps after ties; DENSE_RANK() does not',
          explanation: 'RANK: tied rows get same rank but next rank skips (1,1,3). DENSE_RANK: no gaps (1,1,2).',
        },,
        {
          id: 'sql-window-extra-1',
          type: 'FILL_BLANK',
          question: "Rank rows by salary",
          codeSnippet: "SELECT name, salary,\n___(salary) OVER (ORDER BY salary DESC) AS rank\nFROM employees;",
          correctAnswer: 'RANK',
          explanation: 'RANK() assigns ranks. OVER clause defines the window (partition/order).',
          xpReward: 20, difficulty: 'hard',
        },
        {
          id: 'sql-window-extra-2',
          type: 'MULTIPLE_CHOICE',
          question: "What is the difference between RANK() and DENSE_RANK()?",
          options: [
            'No difference',
            'RANK skips numbers after ties, DENSE_RANK does not',
            'DENSE_RANK is slower',
            'RANK is for strings, DENSE_RANK for numbers'
          ],
          correctAnswer: 'RANK skips numbers after ties, DENSE_RANK does not',
          explanation: 'RANK: 1,1,3. DENSE_RANK: 1,1,2. Dense rank has no gaps after ties.',
          xpReward: 20, difficulty: 'hard',
        },
        {
          id: 'sql-window-extra-3',
          type: 'FILL_BLANK',
          question: "Compute running total with window function",
          codeSnippet: "SELECT date, amount,
  ___(amount) OVER (ORDER BY date) AS running_total
FROM sales;",
          correctAnswer: 'SUM',
          explanation: 'SUM() OVER (ORDER BY ...) computes a running total up to the current row.',
          xpReward: 20, difficulty: 'hard',
        }
      ],
    },
    {
      id: 'sql-boss',
      sectionId: 'sql-aggregates',
      title: 'SQL Boss',
      description: 'The ultimate SQL challenge.',
      lessonType: 'boss' as const,
      order: 99,
      completionXpBonus: 100,
      challenges: [
        {
          id: 'sql-b-1',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'Find the highest earner per department',
          difficulty: 'hard' as const,
          question: 'Which query finds the maximum salary per department?',
          options: [
            'SELECT department, MAX(salary) FROM employees;',
            'SELECT department, MAX(salary) FROM employees GROUP BY department;',
            'SELECT department, salary FROM employees ORDER BY salary DESC;',
            'SELECT department, TOP(salary) FROM employees GROUP BY department;',
          ],
          correctAnswer: 'SELECT department, MAX(salary) FROM employees GROUP BY department;',
          explanation: 'GROUP BY department groups rows, MAX(salary) finds the highest per group.',
        },
        {
          id: 'sql-b-2',
          type: 'FILL_BLANK' as const,
          instruction: 'Get the second highest salary',
          difficulty: 'hard' as const,
          codeWithBlank: 'SELECT MAX(salary) FROM employees\nWHERE salary < (SELECT ___(salary) FROM employees);',
          options: ['MAX', 'MIN', 'AVG', 'TOP'],
          correctAnswer: 'MAX',
          explanation: 'The subquery gets the MAX salary. The outer query gets MAX salary where salary is less than the global max = 2nd highest.',
        },
        {
          id: 'sql-b-3',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'Choose the right JOIN for this query',
          difficulty: 'hard' as const,
          question: 'You want all orders, even for deleted customers (NULL customer). Which JOIN?',
          options: [
            'INNER JOIN customers ON orders.customer_id = customers.id',
            'LEFT JOIN customers ON orders.customer_id = customers.id',
            'RIGHT JOIN customers ON orders.customer_id = customers.id',
            'CROSS JOIN customers',
          ],
          correctAnswer: 'LEFT JOIN customers ON orders.customer_id = customers.id',
          explanation: 'LEFT JOIN keeps all orders (left table). Deleted customers give NULL values in customer columns.',
        },
        {
          id: 'sql-b-4',
          type: 'FILL_BLANK' as const,
          instruction: 'Count customers with more than 3 orders',
          difficulty: 'hard' as const,
          codeWithBlank: 'SELECT customer_id, COUNT(*) AS order_count\nFROM orders\nGROUP BY customer_id\n___ COUNT(*) > 3;',
          options: ['HAVING', 'WHERE', 'FILTER', 'AND'],
          correctAnswer: 'HAVING',
          explanation: 'HAVING filters grouped results. WHERE cannot filter aggregates.',
        },
        {
          id: 'sql-b-5',
          type: 'MULTIPLE_CHOICE' as const,
          instruction: 'What is the correct SQL clause order?',
          difficulty: 'hard' as const,
          question: 'What is the correct order of SQL clauses in a query?',
          options: [
            'SELECT, FROM, WHERE, GROUP BY, HAVING, ORDER BY, LIMIT',
            'FROM, SELECT, WHERE, GROUP BY, ORDER BY, HAVING, LIMIT',
            'SELECT, WHERE, FROM, GROUP BY, HAVING, ORDER BY',
            'FROM, WHERE, SELECT, HAVING, GROUP BY, ORDER BY',
          ],
          correctAnswer: 'SELECT, FROM, WHERE, GROUP BY, HAVING, ORDER BY, LIMIT',
          explanation: 'SQL clause order: SELECT > FROM > JOIN > WHERE > GROUP BY > HAVING > ORDER BY > LIMIT.',
        },,
        {
          id: 'sql-boss-extra-1',
          type: 'MULTIPLE_CHOICE',
          question: "What is a CTE (Common Table Expression)?",
          options: [
            'A type of index',
            'A temporary named result set defined with WITH',
            'A constraint type',
            'A type of join'
          ],
          correctAnswer: 'A temporary named result set defined with WITH',
          explanation: 'WITH cte AS (...) SELECT * FROM cte creates a readable, reusable subquery.',
          xpReward: 20,
          difficulty: 'hard',
        },
        {
          id: 'sql-boss-extra-2',
          type: 'PREDICT_OUTPUT',
          question: "What does COALESCE do?",
          codeSnippet: "SELECT COALESCE(NULL, NULL, 'default', 'other');",
          correctAnswer: 'default',
          explanation: 'COALESCE returns the first non-NULL value. default is the first non-NULL.',
          xpReward: 20, difficulty: 'medium',
        }
      ],
    },
  ],
};

const sqlLanguage: Language = {
  id: 'sql',
  name: 'SQL',
  description: 'Query and manage databases. Essential for every developer.',
  icon: '🗄️',
  status: 'coming_soon',
  sections: [sqlSection1, sqlSection2, sqlSection3],
};

export default sqlLanguage;
